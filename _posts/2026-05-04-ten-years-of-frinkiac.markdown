...
Update: Today (May 4, 2026) we transitioned from GIN to RUM indices and tuned PostgreSQL for a 4-5x speed up in search queries after noticing significant performance issues during the launch. Surprisingly, we needed to use `plan_cache_mode=force_custom_plan` to prevent PostgreSQL from choosing incorrect query plans for the searches that excluded our indices.
